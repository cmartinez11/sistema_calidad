...
<div x-data="{
    ...
    editMolde: {
        ...
        codigo: '{{ old('codigo') }}',
        nombre: '{{ old('nombre') }}',
        numero_cavidades: '{{ old('numero_cavidades') }}',
    ...
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Gestión de Moldes</h2>
            <p class="text-xs text-gray-400 mt-1">Control de inventario de moldes y número de cavidades para inyección de preformas</p>
        </div>

        <div class="flex items-center space-x-3">
            <!-- Botón Búsqueda -->
            <form method="GET" action="{{ route('moldes.index') }}" class="relative">
            ...
            <table class="w-full text-left text-sm text-gray-600">
                <thead class="bg-gray-50 border-b border-gray-100 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-center">N° Cavidades</th>
                    ...
                    @forelse($moldes as $molde)
                        <tr class="hover:bg-gray-50/80 transition-colors">
                            ...
                            <td class="px-6 py-4 text-center whitespace-nowrap">
                                <span class="bg-fenix/10 text-fenix px-3 py-1 rounded-lg text-xs font-mono font-bold border border-fenix/20">
                                    {{ $molde->numero_cavidades }} Cavidades
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4 text-center whitespace-nowrap">
                                @if($molde->activo)
                                    <span class="inline-flex items-center px-3 py-1 bg-green-100 text-green-700 text-xs font-bold rounded-full border border-green-200">
                                        <span class="w-2 h-2 bg-green-500 rounded-full mr-2"></span> HABILITADO
                                    </span>
                                @else
                            ...
                            <td class="px-6 py-4 text-right whitespace-nowrap space-x-2">
                                ...
                                <button @click="
                                            editMolde = {
                                                id: {{ $molde->id }},
                                                codigo: '{{ e($molde->codigo) }}',
                                                nombre: '{{ e($molde->nombre) }}',
                                                numero_cavidades: {{ $molde->numero_cavidades }},
                                                activo: {{ $molde->activo ? 'true' : 'false' }}
                                            };
                                            editUrl = '{{ route('moldes.update', $molde->id) }}';
                                            editModalOpen = true;
                                        " 
                                ...
                                <!-- Botón Eliminar -->
                                <button @click="
                                            deleteUrl = '{{ route('moldes.destroy', $molde->id) }}';
                        ...
                        <tr>
                            <td colspan="5" class="text-center py-12 text-gray-400">
                                <div class="flex flex-col items-center justify-center space-y-2">
                                    ...
                                    <p class="text-base font-medium text-gray-500">No se encontraron moldes registrados</p>
                                    <p class="text-xs text-gray-400">Haz clic en "Nuevo Molde" para agregar el primero.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        <div class="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden border border-gray-100 my-8" @click.away="createModalOpen = false">
            <div class="bg-fenix text-white px-6 py-4 flex items-center justify-between">
                <div class="flex items-center space-x-2">
                    ...
                    <h3 class="text-lg font-bold">Registrar Nuevo Molde</h3>
                </div>
                <button @click="createModalOpen = false" class="text-white/80 hover:text-white text-2xl font-bold">&times;</button>
            </div>

            <form action="{{ route('moldes.store') }}" method="POST" class="p-6 space-y-4">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="codigo" value="{{ old('codigo') }}" placeholder="Ej. MOLD-01" required
                    ...
                    <!-- Número de Cavidades -->
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 uppercase mb-1">N° de Cavidades *</label>
                        <input type="number" name="numero_cavidades" value="{{ old('numero_cavidades') }}" placeholder="Ej. 4" min="1" required
                               class="w-full px-3.5 py-2 border border-gray-300 rounded-xl text-sm focus:outline-none focus:border-fenix focus:ring-1 focus:ring-fenix font-mono font-bold">
                        @error('numero_cavidades') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="nombre" value="{{ old('nombre') }}" placeholder="Ej. Molde Tapa Galón 4 Cav." required
                ...
                <div class="flex items-center space-x-2 pt-2">
                    <input type="checkbox" name="activo" id="create_activo_molde" value="1" {{ old('activo', '1') ? 'checked' : '' }}
                           class="w-4 h-4 text-fenix rounded border-gray-300 focus:ring-fenix">
                    <label for="create_activo_molde" class="text-sm font-medium text-gray-700">Molde Habilitado / Activo</label>
                </div>
                ...
                <div class="flex justify-end space-x-3 pt-4 border-t border-gray-100">
                    ...
                    <button type="submit" 
                            class="px-5 py-2 bg-fenix hover:bg-fenix-dark text-white rounded-xl text-sm font-semibold shadow-md">Guardar Molde</button>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden border border-gray-100 my-8" @click.away="editModalOpen = false">
            <div class="bg-gray-900 text-white px-6 py-4 flex items-center justify-between">
                <div class="flex items-center space-x-2">
                    ...
                    <h3 class="text-lg font-bold">Editar Molde</h3>
                </div>
            ...
            <form :action="editUrl" method="POST" class="p-6 space-y-4">
                @csrf
                @method('PUT')
                <input type="hidden" name="id" :value="editMolde.id">

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <!-- Código -->
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 uppercase mb-1">Código de Molde *</label>
                        <input type="text" name="codigo" x-model="editMolde.codigo" required
                               class="w-full px-3.5 py-2 border border-gray-300 rounded-xl text-sm focus:outline-none focus:border-fenix focus:ring-1 focus:ring-fenix font-mono font-bold">
                    </div>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 uppercase mb-1">N° de Cavidades *</label>
                        <input type="number" name="numero_cavidades" x-model="editMolde.numero_cavidades" min="1" required
                               class="w-full px-3.5 py-2 border border-gray-300 rounded-xl text-sm focus:outline-none focus:border-fenix focus:ring-1 focus:ring-fenix font-mono font-bold">
                    </div>
                </div>
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-700 uppercase mb-1">Nombre / Descripción *</label>
                    <input type="text" name="nombre" x-model="editMolde.nombre" required
                           class="w-full px-3.5 py-2 border border-gray-300 rounded-xl text-sm focus:outline-none focus:border-fenix focus:ring-1 focus:ring-fenix font-medium">
                </div>

                <!-- Activo Checkbox -->
                <div class="flex items-center space-x-2 pt-2">
                    <input type="checkbox" name="activo" id="edit_activo_molde" value="1" :checked="editMolde.activo"
                           class="w-4 h-4 text-fenix rounded border-gray-300 focus:ring-fenix">
                    <label for="edit_activo_molde" class="text-sm font-medium text-gray-700">Molde Habilitado / Activo</label>
                </div>
                ...
                <div class="flex justify-end space-x-3 pt-4 border-t border-gray-100">
                    <button type="button" @click="editModalOpen = false" 
                            class="px-4 py-2 border border-gray-300 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50">Cancelar</button>
                    <button type="submit" 
                            class="px-5 py-2 bg-fenix hover:bg-fenix-dark text-white rounded-xl text-sm font-semibold shadow-md">Actualizar Molde</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div x-show="deleteModalOpen" x-cloak 
        ...
        <div class="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden border border-gray-100" @click.away="deleteModalOpen = false">
            <div class="p-6 text-center space-y-4">
                ...
                <h3 class="text-lg font-bold text-gray-800">¿Eliminar Molde?</h3>
                <p class="text-sm text-gray-500">Esta acción no se puede deshacer. Si el molde tiene inspecciones registradas, no podrá ser eliminado.</p>
