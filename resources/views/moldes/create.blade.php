...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-gray-100 space-y-6">
        ...
        <div class="flex justify-between items-center border-b border-gray-100 pb-4">
            <div>
                <h2 class="text-xl font-bold text-gray-800 tracking-tight">
                    {{ isset($molde) ? 'Editar Molde' : 'Registrar Nuevo Molde' }}
                </h2>
                <p class="text-xs text-gray-400 mt-1">
                    {{ isset($molde) ? 'Modifica los datos del molde existente' : 'Ingresa la información técnica del nuevo molde' }}
                </p>
            </div>
        ...
        <form action="{{ isset($molde) ? route('moldes.update', $molde->id) : route('moldes.store') }}" method="POST" class="space-y-5">
            @csrf
            @if(isset($molde))
                @method('PUT')
            @endif
            ...
            <div class="flex items-center space-x-2 pt-2">
                <input type="checkbox" name="activo" id="activo_molde_form" value="1" 
                       {{ old('activo', $molde->activo ?? true) ? 'checked' : '' }}
                       class="w-4 h-4 text-fenix rounded border-gray-300 focus:ring-fenix">
                <label for="activo_molde_form" class="text-sm font-medium text-gray-700">Molde Habilitado / Activo</label>
            </div>
            ...
            <div class="flex items-center justify-end space-x-3 pt-6 border-t border-gray-100">
                ...
                <button type="submit" 
                    ...
                    {{ isset($molde) ? 'Actualizar Molde' : 'Guardar Molde' }}
                </button>
            </div>
        </form>
    </div>
