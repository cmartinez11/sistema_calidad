...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-gray-100 space-y-6">
        ...
        <div class="flex justify-between items-center border-b border-gray-100 pb-4">
            <div>
                <h2 class="text-xl font-bold text-gray-800 tracking-tight">
                    {{ isset($resina) ? 'Editar Resina' : 'Registrar Nueva Resina' }}
                </h2>
                <p class="text-xs text-gray-400 mt-1">
                    {{ isset($resina) ? 'Modifica los datos de la resina existente' : 'Ingresa la información técnica de la nueva resina' }}
                </p>
            </div>
        ...
        <form action="{{ isset($resina) ? route('resinas.update', $resina->id) : route('resinas.store') }}" method="POST" class="space-y-5">
            @csrf
            @if(isset($resina))
                @method('PUT')
            @endif
            ...
            <div class="flex items-center space-x-2 pt-2">
                <input type="checkbox" name="activo" id="activo_resina_form" value="1" 
                       {{ old('activo', $resina->activo ?? true) ? 'checked' : '' }}
                       class="w-4 h-4 text-fenix rounded border-gray-300 focus:ring-fenix">
                <label for="activo_resina_form" class="text-sm font-medium text-gray-700">Resina Habilitada / Activa</label>
            </div>
            ...
            <div class="flex items-center justify-end space-x-3 pt-6 border-t border-gray-100">
                ...
                <button type="submit" 
                    ...
                    {{ isset($resina) ? 'Actualizar Resina' : 'Guardar Resina' }}
                </button>
            </div>
        </form>
    </div>
