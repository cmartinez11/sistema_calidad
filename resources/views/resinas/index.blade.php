...
<div x-data="{
    ...
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Gestión de Resinas</h2>
            <p class="text-xs text-gray-400 mt-1">Catálogo de materias primas y tipos de resinas empleadas en inyección</p>
        </div>

        <div class="flex items-center space-x-3">
            <!-- Botón Búsqueda -->
            <form method="GET" action="{{ route('resinas.index') }}" class="relative">
            ...
            <table class="w-full text-left text-sm text-gray-600">
                <thead class="bg-gray-50 border-b border-gray-100 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                    <tr>
                        ...
                        <th class="px-6 py-4">Nombre de la Resina</th>
                        <th class="px-6 py-4 text-center">Estado</th>
                        <th class="px-6 py-4 text-right">Acciones</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @forelse($resinas as $resina)
                        <tr class="hover:bg-gray-50/80 transition-colors">
                            ...
                            <td class="px-6 py-4 text-center whitespace-nowrap">
                                @if($resina->activo)
                                    <span class="inline-flex items-center px-3 py-1 bg-green-100 text-green-700 text-xs font-bold rounded-full border border-green-200">
                                        <span class="w-2 h-2 bg-green-500 rounded-full mr-2"></span> HABILITADA
                                    </span>
                                @else
                            ...
                            <td class="px-6 py-4 text-right whitespace-nowrap space-x-2">
                                ...
                                <button @click="
                                            editResina = {
                                                id: {{ $resina->id }},
                                                codigo: '{{ e($resina->codigo) }}',
                                                nombre: '{{ e($resina->nombre) }}',
                                                activo: {{ $resina->activo ? 'true' : 'false' }}
                                            };
                                            editUrl = '{{ route('resinas.update', $resina->id) }}';
                                            editModalOpen = true;
                                        " 
                                ...
                                <!-- Botón Eliminar -->
                                <button @click="
                                            deleteUrl = '{{ route('resinas.destroy', $resina->id) }}';
                        ...
                        <tr>
                            <td colspan="4" class="text-center py-12 text-gray-400">
                                <div class="flex flex-col items-center justify-center space-y-2">
                                    ...
                                    <p class="text-base font-medium text-gray-500">No se encontraron resinas registradas</p>
                                    <p class="text-xs text-gray-400">Haz clic en "Nueva Resina" para agregar la primera.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        <div class="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden border border-gray-100 my-8" @click.away="createModalOpen = false">
            ...
            <form action="{{ route('resinas.store') }}" method="POST" class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="nombre" value="{{ old('nombre') }}" placeholder="Ej. Polipropileno Homopolímero" required
                ...
                <div class="flex items-center space-x-2 pt-2">
                    <input type="checkbox" name="activo" id="create_activo_resina" value="1" {{ old('activo', '1') ? 'checked' : '' }}
                           class="w-4 h-4 text-fenix rounded border-gray-300 focus:ring-fenix">
                    <label for="create_activo_resina" class="text-sm font-medium text-gray-700">Resina Habilitada / Activa</label>
                </div>
                ...
                <div class="flex justify-end space-x-3 pt-4 border-t border-gray-100">
                    ...
                    <button type="submit" 
                            class="px-5 py-2 bg-fenix hover:bg-fenix-dark text-white rounded-xl text-sm font-semibold shadow-md">Guardar Resina</button>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden border border-gray-100 my-8" @click.away="editModalOpen = false">
            <div class="bg-gray-900 text-white px-6 py-4 flex items-center justify-between">
                <div class="flex items-center space-x-2">
                    ...
                    <h3 class="text-lg font-bold">Editar Resina</h3>
                </div>
            ...
            <form :action="editUrl" method="POST" class="p-6 space-y-4">
                @csrf
                @method('PUT')
                <input type="hidden" name="id" :value="editResina.id">

                <!-- Código -->
                <div>
                    <label class="block text-xs font-semibold text-gray-700 uppercase mb-1">Código de Resina *</label>
                    <input type="text" name="codigo" x-model="editResina.codigo" required
                           class="w-full px-3.5 py-2 border border-gray-300 rounded-xl text-sm focus:outline-none focus:border-fenix focus:ring-1 focus:ring-fenix font-mono font-bold">
                </div>
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-700 uppercase mb-1">Nombre de la Resina *</label>
                    <input type="text" name="nombre" x-model="editResina.nombre" required
                           class="w-full px-3.5 py-2 border border-gray-300 rounded-xl text-sm focus:outline-none focus:border-fenix focus:ring-1 focus:ring-fenix font-medium">
                </div>

                <!-- Activo Checkbox -->
                <div class="flex items-center space-x-2 pt-2">
                    <input type="checkbox" name="activo" id="edit_activo_resina" value="1" :checked="editResina.activo"
                           class="w-4 h-4 text-fenix rounded border-gray-300 focus:ring-fenix">
                    <label for="edit_activo_resina" class="text-sm font-medium text-gray-700">Resina Habilitada / Activa</label>
                </div>
                ...
                <div class="flex justify-end space-x-3 pt-4 border-t border-gray-100">
                    <button type="button" @click="editModalOpen = false" 
                            class="px-4 py-2 border border-gray-300 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50">Cancelar</button>
                    <button type="submit" 
                            class="px-5 py-2 bg-fenix hover:bg-fenix-dark text-white rounded-xl text-sm font-semibold shadow-md">Actualizar Resina</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div x-show="deleteModalOpen" x-cloak 
        ...
        <div class="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden border border-gray-100" @click.away="deleteModalOpen = false">
            <div class="p-6 text-center space-y-4">
                ...
                <h3 class="text-lg font-bold text-gray-800">¿Eliminar Resina?</h3>
                <p class="text-sm text-gray-500">Esta acción no se puede deshacer. Si la resina está asociada a inspecciones registradas, no podrá ser eliminada.</p>
